...
function installRuleAndSkill() {
  ...
  const rule = line(MARKER, "# minitok MCP routing", "", "For repository coding tasks, use the minitok MCP workflow before direct editor or shell tools.", "", "- For changes, call minitok_run with the absolute repository path and task.", "- Use minitok_status or read-only minitok tools for inspection.", "- Do not silently bypass minitok with direct file or shell tools when a minitok tool is available.", "- If minitok MCP is unavailable, tell the user instead of pretending the verified workflow ran.", "- Keep auto_accept disabled unless the user explicitly requests it.");
  const skill = line("---", "name: minitok", "description: Run repository coding tasks through minitok MCP planning, implementation, verification, and evidence. Use for coding, bug fixes, refactors, and repository changes.", "---", MARKER, "# minitok MCP workflow", "", "1. Use the minitok MCP server.", "2. For repository changes, call minitok_run with the absolute repository path and the user's task.", "3. Inspect the returned plan, verification, evidence, and approval state.", "4. Do not replace the minitok call with direct editor or shell tools.", "5. Keep auto_accept disabled unless explicitly requested.");
