...
async function startGoal(args, runtimeOptions = {}) {
  ...
  const prepared = prepareGoalExecution({ goal_spec: compiledRecord.spec, mode: policyDecision.mode, capabilities: policyDecision.capabilities, explicit_confirmation: args.mode === "unrestricted" ? args.confirm_unrestricted === true : args.explicit_confirmation === true, auto_accept: policyDecision.audit_context.auto_accept, actor: runtimeOptions.actor, config, source: "mcp" });
  if (prepared.status !== "ready") return resultForSession({ goalSpec: compiledRecord.spec, state: { status: prepared.status } }, { state: prepared.status, current_state: prepared.status, goal_plan: prepared.goal_plan, questions: prepared.questions || [], errors: prepared.errors || [], inferred_steps: prepared.inferred_steps || [], optional_steps: prepared.optional_steps || [], out_of_scope_candidates: prepared.out_of_scope_candidates || [], requires_user_confirmation: prepared.requires_user_confirmation });
...
async function continueGoal(args, runtimeOptions = {}) {
  ...
  const prepared = prepareGoalExecution({ goal_spec: session.goalSpec, existing_goal_plan: session.state.goal_plan, mode: policyDecision.mode, capabilities: policyDecision.capabilities, explicit_confirmation: args.mode === "unrestricted" ? args.confirm_unrestricted === true : args.explicit_confirmation === true, auto_accept: policyDecision.audit_context.auto_accept, actor: runtimeOptions.actor, config, source: "mcp" });
