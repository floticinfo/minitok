...
function persistGeneralLoopState(session, loopState = {}, eventType = "general_loop_checkpoint") {
  ...
  session.state.current_task = projection.current_step || null;
  session.state.completed_steps = projection.completed_steps || session.state.completed_steps || [];
  session.state.failed_steps = projection.failed_steps || session.state.failed_steps || [];
  session.state.action_history = projection.action_history || session.state.action_history || [];
  session.state.task_history = projection.task_history || session.state.task_history || [];
  session.state.evaluator_results = projection.evaluator_results || session.state.evaluator_results || [];
  session.state.verification_results = projection.verification_results || session.state.verification_results || session.state.evaluator_results || [];
  session.state.verifiers = projection.verifiers || session.state.verifiers || [];
  session.state.verifier_results = projection.verifier_results || session.state.verifier_results || [];
  session.state.verifier_fingerprints = projection.verifier_fingerprints || session.state.verifier_fingerprints || {};
  session.state.invalidated_evidence = projection.invalidated_evidence || session.state.invalidated_evidence || [];
  session.state.verification_conflict = projection.verification_conflict === true;
  session.state.evidence_refs = (Array.isArray(projection.evidence) ? projection.evidence : []).map(item => item?.evidence_id).filter(Boolean);
  session.state.verification_required = projection.verification_required === true || (session.state.verification_required === true && projection.state !== "completed");
