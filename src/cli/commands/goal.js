...
async function cmdGoalStart(goal, options = {}) {
  ...
  const session = createGoalSession({ workspaceRoot: repoPath(options), goalSpec: effectiveGoalSpec, model: options.model, provider: options.provider, approvalState: policyDecision.approval_required ? "approval_required" : "not_requested" });
