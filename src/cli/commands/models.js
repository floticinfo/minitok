      ...
      try {
        ...
        if (opts.discover) {
          ...
          for (const [prov, liveIds] of Object.entries(result.live)) {
            ...
            const status = provAvail ? "available" : "unavailable";
            console.log(`${prov.toUpperCase()} (${liveIds.length} models, ${status})`);
          ...
          for (const custom of result.custom) {
            ...
            for (const m of custom.models) {
              ...
              const reasoning = m.reasoning?.supported ? " reasoning=supported" : "";
          ...
          if (result.unknown.length > 0) {
            console.log(`Warning: ${result.unknown.length} model(s) found in API but not in catalog:`);
          ...
          for (const [prov, modelList] of Object.entries(grouped)) {
            const provAvail = available.includes(prov);
            const status = provAvail ? "available" : "unavailable";
            console.log(`${prov.toUpperCase()} (${modelList.length} models, ${status})`);
