...
| `https://minitok.dev` install command | "npm install -g @flotic/minitok@1.3.12" | Point it at the version actually published to npm; 1.3.16 is the version prepared here |

**Version strings.** The homepage still advertises the 1.3.12 install command while
this repository prepares 1.3.16, so the command has to follow the publish. The
Extension was repackaged so its embedded CLI runtime matches 1.3.16 and reports 0.2.8,
...
same URL for 0.2.6, 0.2.7, and 0.2.8 all return HTTP 404, so 0.2.5 is the published
version and none of 0.2.6, 0.2.7, or 0.2.8 has been used. Publish the repackaged VSIX
as 0.2.8, and bump the Extension version whenever a version that was already published
has to change.
