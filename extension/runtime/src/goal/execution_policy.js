...
const EXECUTION_POLICY_MODES = Object.freeze(["safe", "supervised", "authorized_external", "unrestricted", "always_blocked"]);
...
function resolveExecutionPolicy(input = {}) {
  ...
  const validation = validateCapabilities(requested);
  ...
  } else if (normalized.mode === "unrestricted") {
  ...
  return { ...baseDecision(normalized.mode, requested, context), allowed, approval_required: uniqueApproval.length > 0, denied_capabilities: uniqueDenied, always_blocked_capabilities: [], reason: allowed ? (normalized.mode === "unrestricted" ? "explicit unrestricted authorization allows the requested capabilities" : "capabilities are allowed by the current execution policy") : uniqueApproval.length ? "explicit approval is required for one or more capabilities" : "one or more capabilities are denied by the current execution policy" };
}
...
module.exports = { EXECUTION_POLICY_MODES, LEGACY_MODE_ALIASES, SAFE_CAPABILITIES, EXTERNAL_CAPABILITIES, MUTATION_CAPABILITIES, SIDE_EFFECT_CAPABILITY_MAP, normalizeMode, capabilitiesForSideEffects, resolveExecutionPolicy, assertExecutionAllowed };
